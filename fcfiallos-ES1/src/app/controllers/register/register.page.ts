...
export class RegisterPage {
  ...
  validarContrasena(contrasena: string): string[] {
    ...
    const minusculas = contrasena.match(regexMinuscula);
    if (!minusculas || minusculas.length > 3) {
      errores.push('La contraseña debe tener como máximo 3 letras minúsculas.');
      return errores;
    }

    const numeros = contrasena.match(regexNumero);
    if (!numeros || numeros.length !== 3) {
      errores.push('La contraseña debe tener exactamente 3 números.');
      return errores;
    }

    if (!regexFinal.test(contrasena)) {
      errores.push('La contraseña debe terminar con uno de los siguientes caracteres: #@_!');
      return errores;
    }
